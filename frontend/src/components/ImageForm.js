...
const ImageForm = ({setFormOn}) => {
   ...
   const handleSubmit = async (e) => {
      ...
      const res = await fetch(`${process.env.REACT_APP_API_URL}/image/post` , {
        method : 'POST',
    ...
    <div className="bg-white shadow-xl absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30  p-6 flex flex-col content-around  w-5/6 sm:w-[450px] rounded-md">
       ...
       <form onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end mt-10 ">
          ...
          <button type="submit" className="bg-green font-semibold sm:px-4  sm:py-2 px-3 py-1.5 rounded-lg shadow-md md:mr-2 mr-1 text-white text-sm md:text-md hover:opacity-80 duration-300">Submit</button>
        </div>
       </form>
    </div>
